			...
			<div className="bg-slate-100  p-4 pt-10 -mt-16 rounded-xl  w-80 flex flex-col items-center drop-shadow-xl ">
				...
				<div className={loginWithUsername === true ? "w-52 h-52 rounded-lg mb-4" : " hidden"}>
					...
					<div className="flex flex-col pb-3">
						...
						<input ref={emailRef} type="text" name="email" id="" className="rounded-full h-8" />
					</div>

					<div className="flex flex-col pb-3">
						<label htmlFor="" className="text-yellow-500">Password</label>
						<input ref={passwordRef} type="text" name="email" id="" className="rounded-full h-8" />
					</div>
					<button className="bg-yellow-500 w-full rounded-full py-2 text-white" onClick={handleLogIn}>Login</button>
				</div>
				<div className={loginWithUsername === false ? "bg-green-600 w-52 h-52 rounded-lg mb-4" : "hidden"}>
					scanner
				</div>
